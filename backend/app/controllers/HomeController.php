...
class HomeController extends Action
{
    ...
    public function pageLogin()
    {
        ...
        $this->dados->title = 'Painel Administrativo - Origin';
    ...
    public function login()
    {
        ...
        if (filter_has_var(INPUT_POST, 'btnLogar')) :
            ...
            $recaptcha_value = trim(filter_input(INPUT_POST, 'g-recaptcha-response', FILTER_SANITIZE_SPECIAL_CHARS));
            ...
            */
            if (!empty($_SESSION['_bloqueio_login']) || filter_has_var(INPUT_COOKIE, '_bloqueio_login')) {
                ...
                if (!empty($tempo_bloqueio)) {
                    ...
                    if ($data_atual->getTimestamp() < $data_bloqueio->getTimestamp()) {
                        $this->setRetorno('Acesso bloqueado temporariamente até as ' . $data_bloqueio->format('H:i:s'), true, false);
                    } else {
            ...
            if (!empty($continuar_login)) {
                ...
                if ($this->checkReCaptcha($recaptcha_value)) {
    ...
    private function checkReCaptcha($recaptcha_value)
    {
        if (LOCALHOST)
            return true;
        ...
        if (!empty($recaptcha)) {
            ...
            $resp_recaptcha = $recaptcha->verify($recaptcha_value, $_SERVER['REMOTE_ADDR']);
